...
{
    cMin    = CreateConVar("rrm_size_shrink_kill_min",    "0.25", "Minimum size a player can shrink to.");
    cMax    = CreateConVar("rrm_size_shrink_kill_max",    "1.0",  "Maximum size (starting size) for players.");
    cAmount = CreateConVar("rrm_size_shrink_kill_amount", "0.1",  "Amount to shrink per kill.");
    ...
    AutoExecConfig(true, "rrm_resize_shrink_kill", "rrm");
}
...
{
    RRM_Register("Shrink on Kill", 0.0, 0.0, false, RRM_Callback_ShrinkKill);
}
...
public int RRM_Callback_ShrinkKill(bool enable, float value)
{
    gEnabled = enable;
    if(gEnabled)
        SetSize();
...
public void OnPlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
    ...
    {
        gPlayerSize[client] = gMax;
        SetEntPropFloat(client, Prop_Send, "m_flModelScale", gMax);
    }
}
...
void SetSize()
{
    for(int i = 1; i <= MaxClients; i++)
    {
        if(!IsClientInGame(i))
            continue;
        gPlayerSize[i] = gMax;
        SetEntPropFloat(i, Prop_Send, "m_flModelScale", gMax);
    }
}

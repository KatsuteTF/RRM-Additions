...
#define ROCKET_SPEED 1100.0

public Plugin myinfo =
{
    name = "[RRM] Rocket Shot Modifier",
    author = "Katsute",
    description = "Modifier that randomly replaces shots with rockets.",
    version = "1.0"
};

public void OnPluginStart()
{
    cMin = CreateConVar("rrm_rocket_shot_min", "0.1", "Minimum value for the random number generator.");
    cMax = CreateConVar("rrm_rocket_shot_max", "1.0", "Maximum value for the random number generator.");
    ...
    AutoExecConfig(true, "rrm_rocket_shot", "rrm");
...
{
    RRM_Register("Rocket Shot", gMin, gMax, false, RRM_Callback_RocketShot);
}
...
public int RRM_Callback_RocketShot(bool enable, float value)
{
    gEnabled = enable;
    if(gEnabled)
        gChance = value;
...
void SpawnRocket(int client)
{
    ...
    float vel[3];
    vel[0] = forward[0] * ROCKET_SPEED;
    vel[1] = forward[1] * ROCKET_SPEED;
    vel[2] = forward[2] * ROCKET_SPEED;
    ...
    if(weapon != -1)
        SetEntPropEnt(rocket, Prop_Send, "m_hLauncher", weapon);

...
{
    name = "[RRM] Jarate Modifier",
    author = "Katsute",
    description = "Modifier that grants chance of jarate.",
    version = "1.0"
};

public void OnPluginStart()
{
    cMin = CreateConVar("rrm_jarate_min", "0.1", "Minimum value for the random number generator.");
    cMax = CreateConVar("rrm_jarate_max", "1.0", "Maximum value for the random number generator.");
    cDuration = CreateConVar("rrm_jarate_duration", "3.0", "Duration for jarate to last on affected players.");
    ...
    AutoExecConfig(true, "rrm_jarate", "rrm");
}
...
{
    RRM_Register("Jarate", gMin, gMax, false, RRM_Callback_Jarate);
}
...
public int RRM_Callback_Jarate(bool enable, float value)
{
    gEnabled = enable;
    if(gEnabled)
        gChance = value;
    ...
    float damageForce[3], float damagePosition[3], int damagecustom)
{
    ...
    if(gChance > RandomFloat(RandomFloat(0.0, 1.0)))
    {
        ...
        if(!TF2_IsPlayerInCondition(victim, TFCond_Jarated))
            TF2_AddCondition(victim, TFCond_Jarated, gDuration, attacker);
    }
    return Plugin_Continue;
}

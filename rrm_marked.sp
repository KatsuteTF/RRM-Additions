...
{
    name = "[RRM] Marked for Death Modifier",
    author = "Katsute",
    description = "Modifier that grants chance of marked for death.",
    version = "1.0"
};

public void OnPluginStart()
{
    cMin = CreateConVar("rrm_marked_min", "0.1", "Minimum value for the random number generator.");
    cMax = CreateConVar("rrm_marked_max", "1.0", "Maximum value for the random number generator.");
    cDuration = CreateConVar("rrm_marked_duration", "3.0", "Duration for marked for death to last on affected players.");
    ...
    AutoExecConfig(true, "rrm_marked", "rrm");
}
...
{
    RRM_Register("Marked for Death", gMin, gMax, false, RRM_Callback_Marked);
}
...
public int RRM_Callback_Marked(bool enable, float value)
{
    gEnabled = enable;
    if(gEnabled)
        gChance = value;
    ...
    float damageForce[3], float damagePosition[3], int damagecustom)
{
    ...
    if(gChance > RandomFloat(RandomFloat(0.0, 1.0)))
    {
        ...
        if(!TF2_IsPlayerInCondition(victim, TFCond_MarkedForDeath))
            TF2_AddCondition(victim, TFCond_MarkedForDeath, gDuration, attacker);
    }
    return Plugin_Continue;
}

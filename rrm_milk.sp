...
{
    name = "[RRM] Mad Milk Modifier",
    author = "Katsute",
    description = "Modifier that grants chance of mad milk.",
    version = "1.0"
};

public void OnPluginStart()
{
    cMin = CreateConVar("rrm_milk_min", "0.1", "Minimum value for the random number generator.");
    cMax = CreateConVar("rrm_milk_max", "1.0", "Maximum value for the random number generator.");
    cDuration = CreateConVar("rrm_milk_duration", "3.0", "Duration for mad milk to last on affected players.");
    ...
    AutoExecConfig(true, "rrm_milk", "rrm");
}
...
{
    RRM_Register("Mad Milk", gMin, gMax, false, RRM_Callback_Milk);
}
...
public int RRM_Callback_Milk(bool enable, float value)
{
    gEnabled = enable;
    if(gEnabled)
        gChance = value;
    ...
    float damageForce[3], float damagePosition[3], int damagecustom)
{
    ...
    if(gChance > RandomFloat(RandomFloat(0.0, 1.0)))
    {
        ...
        if(!TF2_IsPlayerInCondition(victim, TFCond_Milked))
            TF2_AddCondition(victim, TFCond_Milked, gDuration, attacker);
    }
    return Plugin_Continue;
}

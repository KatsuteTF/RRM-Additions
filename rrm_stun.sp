...
{
    name = "[RRM] Stun Modifier",
    author = "Katsute",
    description = "Modifier that grants chance of stun.",
    version = "1.0"
};

public void OnPluginStart()
{
    cMin = CreateConVar("rrm_stun_min", "0.1", "Minimum value for the random number generator.");
    cMax = CreateConVar("rrm_stun_max", "1.0", "Maximum value for the random number generator.");
    cDuration = CreateConVar("rrm_stun_duration", "3.0", "Duration for stun to last on affected players.");
    ...
    AutoExecConfig(true, "rrm_stun", "rrm");
}
...
{
    RRM_Register("Stun", gMin, gMax, false, RRM_Callback_Stun);
}
...
public int RRM_Callback_Stun(bool enable, float value)
{
    gEnabled = enable;
    if(gEnabled)
        gChance = value;
    ...
    float damageForce[3], float damagePosition[3], int damagecustom)
{
    ...
    if(gChance > RandomFloat(RandomFloat(0.0, 1.0)))
    {
        ...
        if(!TF2_IsPlayerInCondition(victim, TFCond_Dazed))
            TF2_StunPlayer(victim, gDuration, 0.0, TF_STUNFLAGS_BIGBONK, attacker);
    }
    return Plugin_Continue;
}

...
#define BAT_SPEED 800.0

public Plugin myinfo =
{
    name = "[RRM] Bat Shot Modifier",
    author = "Katsute",
    description = "Modifier that randomly replaces shots with bats.",
    version = "2.0"
};
...
public void OnPluginStart()
{
    ...
    AutoExecConfig(true, "rrm_shoot_bats", "rrm");
}
...
{
    RRM_Register("Bat Shot", gMin, gMax, false, RRM_Callback_BatShot);
}
...
public int RRM_Callback_BatShot(bool enable, float value)
{
    gEnabled = enable;
    if(gEnabled)
        gChance = value;
...
void SpawnBat(int client)
{
    ...
    float vel[3];
    vel[0] = fwd[0] * BAT_SPEED;
    vel[1] = fwd[1] * BAT_SPEED;
    vel[2] = fwd[2] * BAT_SPEED;

    int proj = CreateEntityByName("tf_projectile_spellbats");
    if(proj == -1)
        return;
